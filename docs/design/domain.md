...
- 주문, 배송, 주문 상품에서 deleted 추가 //결제에도 추가  
  Soft Delete하기 위해 추가
...
- 카테고리에서 child 필드 이름 children으로 변경
  자식 카테고리는 N이므로 복수형으로 변경
...
- 주문 상품에서 수량을 count에서 quantity로 변경  
  개수 느낌에서 수량 느낌으로 변경
...
- 카테고리 categoryNumber 추가  
  카테고리 이름을 유니크 키에서 부모 카테고리와 복합 유니크 키로 변경하기 위해 추가
