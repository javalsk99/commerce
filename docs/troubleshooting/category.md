...
## 운영 중 발생한 문제
- 카테고리 이름을 유니크 키로 설정해 부모 카테고리가 다를 때, 카테고리 이름을 사용할 수 없다 (국내 영화 - 코미디, 해외 영화 - 코미디)  
  일부 해결: 카테고리 이름을 유니크 키에서 해제하고, 부모 카테고리와 카테고리 이름을 복합 유니크 키로 설정하고, 카테고리 번호를 유니크 키로 추가해서 @PathVariable을 카테고리 이름에서 카테고리 번호로 변경한다.  
  추가 문제: 부모 카테고리가 Null인 경우, 카테고리 이름이 중복되어도 추가로 저장된다. (이게 추가로 문제가 되는 이유가 카테고리의 부모 카테고리 변경이 자유롭기 때문에 중복 카테고리가 많이 생길 수가 있다.)

  해결: 서비스에서 부모 카테고리가 Null인 경우도, 카테고리 이름이 중복되면 중복 예외가 발생하게 설정했다.
